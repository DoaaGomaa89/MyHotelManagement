...
def index(request):
    ...
    # Initialize hotel_images as an empty list
    context['hotel_images'] = [] 
    
    # This custom method might not exist, so let's use a standard Django query
    # hotel = Hotel.get_hotel() 
    hotel = Hotel.objects.first() # Get the first Hotel object, if any

    # Only try to get images if a hotel object was found
    if hotel:
